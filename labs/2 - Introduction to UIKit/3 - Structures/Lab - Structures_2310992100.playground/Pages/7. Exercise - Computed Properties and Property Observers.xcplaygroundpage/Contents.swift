...
var ankietsHouse = Rectangle(width: 10000000, height: 20000000)
ankietsHouse.area()
...
struct Height {
    var heightInInches: Double {
        didSet {
            let expected = heightInInches * 2.54
            if abs(heightInCentimeters - expected) > 0.001 {
                heightInCentimeters = expected
            }
        }
    }
    ...
    var heightInCentimeters: Double {
        didSet {
            let expected = heightInCentimeters / 2.54
            if abs(heightInInches - expected) > 0.001 {
                heightInInches = expected
            }
        }
    }
